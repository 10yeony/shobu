...
public class PlayerUpdate {
		...
		for(int i = 0; i<4; i++) {
			try {
				...
				for (int j = 0 ; j<links.select("a").size(); j++) {
					...
					teamName = links.select("a").eq(j).toString().split("'")[1];
					...
					System.out.println(position + " " + name +" "+ teamName +" " + playerId);
				}
	...
	//10.투수성적 업데이트
	public PitcherVO updatePitcher(int playerId, String teamName) {
		PitcherVO pitcher = new PitcherVO();
		try {
			Document doc = Jsoup.connect("https://betman.co.kr/sportsPlayerIntro.so?method=inquirePlayerIntro&item=BS&league=BS001&seq=&id=1&teamId="+teamName+"&isToto=&playerId="+playerId).get();
	...
	//11.타자성적 업데이트
	public HitterVO updateHitter(int playerId, String teamName) {
	...
	public ArrayList<TeamVO> updateTeam(){
		...
		try {
			for(int i = 0; i<10; i++) {
				...
				Document doc = Jsoup.connect("https://betman.co.kr/teamrankinging.so?method=inquirerankinging&item=BS&league=BS001&id=1&fromCode=left").get();
				Elements links = doc.select(".dataH01 tbody tr").eq(i);
				
				team.setRanking(Integer.parseInt(links.select("td").eq(0).text()));
				team.setTeamCode(links.select("td").eq(1).text());
