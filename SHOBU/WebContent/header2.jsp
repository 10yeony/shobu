...
<head>
  ...
  <script>
    ...
    $(function(){
    	...
    	/* 토토 결과 보기 */
    	var id = '${member.id}'; //아이디
    	var temp = new Date();   
		var year = temp.getFullYear(); //년도
		var month = temp.getMonth() + 1; //월
		var date = temp.getDate() - 1; //날짜
		if(month<10) month = "0"+month;
		if(date<10) date = "0"+date;
		const yesterday = year+"-"+month+"-"+date; //어제
		$('#yesterday').text(yesterday);
		var flag = false; //어제 날짜에 투표했는지 아닌지
      ...
      <span class="w3-hide-large w3-right w3-animate-opacity" style="position: relative; top: 20%;">
      	<c:choose>
			<c:when test="${member != null}">
				...
				<span> | </span>
				<a class="menu totoResult" href="#" onclick="document.getElementById('id02').style.display='block'">
				  결과 보기
				</a>
			</c:when>
		</c:choose>
        ...
        <div class="w3-modal-content w3-card-4 w3-animate-zoom" style="max-width:90%">
          ...
          <div class="w3-section" style="padding:40px;">
            <h6 style="text-align:center;">
              <b>
                <span id="yesterday"></span> 경기
              </b>
                <span>&nbsp;( <span id="correctCount"></span>개 정답 / <span id="totalCount"></span>개 선택 )</span>
            </h6>
